...
argparser.add_argument('-d', '--date', dest='date', action='store_true',
                       help='Вписывать в имена файлов дату переименования')
argparser.add_argument('-n', '--name', metavar='[None]', dest='name', type=str,
                       help='Общая для всех файлов часть имени')
...
for src_file_name in os.listdir(args.src_dir_path):
        ...
        if args.name != None:
                trg_file_draft.insert(1, args.name)
